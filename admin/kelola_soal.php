            ...
            <div class="question-card bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden hover:shadow-md transition-shadow" data-no="<?= (int)$s['nomor_soal'] ?>">
                ...
                <div class="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 bg-slate-50">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <a href="#" onclick="confirmDeleteQuestion(<?= $s['id'] ?>); return false;"
...
<script>
    ...
    async function confirmDeleteQuestion(questionId) {
        const confirmed = await showNotification('Hapus Soal', 'Hapus soal ini? Tindakan ini tidak bisa dibatalkan.', 'error', true);
        if (confirmed) {
            window.location.href = 'hapus_soal.php?id=' + encodeURIComponent(questionId);
        }
        return false;
    }
